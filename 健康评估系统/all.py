...
def health_system_app():
    ...
    height = st.number_input("身高 (cm)", 50.0, 250.0, 170.0)
    weight = st.number_input("体重 (kg)", 10.0, 300.0, 65.0)

    st.subheader("健康指标")
    sbp = st.number_input("收缩压 SBP (mmHg)", 50, 250, 120)
    dbp = st.number_input("舒张压 DBP (mmHg)", 30, 150, 80)
    hr = st.number_input("静息心率 (bpm)", 30, 200, 70)
    waist = st.number_input("腰围 (cm)", 30.0, 200.0, 75.0)
    ...
    if st.button("开始评估"):
        ...
        st.write(f"**年龄**：{age} 岁")
        st.write(f"**BMI：{bmi:.2f}** —— 分类：{bmi_status}")
        st.write(f"**血压：{sbp}/{dbp} mmHg** —— 分类：{bp_status}")
        st.write(f"**心率：{hr} bpm** —— {hr_status}")
        st.write(f"**腰围：{waist} cm** —— {waist_flag}")
        ...
        ax.text(bmi, 0.9, f"BMI={bmi:.1f}", rotation=90, va='bottom', ha='center', fontsize=9)
        ax.set_yticks([])
        ax.set_xlabel("BMI 值")
...
def diabetes_model_app():
    ...
    if st.button("评估风险"):
        try:
            ...
            st.success(f"预测结果：{'高风险（可能患糖尿病）' if label == 1 else '低风险（可能性较低）'}")
            st.write(f"患病概率（风险评分）：{prob:.4f}")
        except:
            st.error("请填写所有指标，并确保为有效数字。")
